 ...
 describe('GitHub app', () => {
    ...
    it('should behave well with individual configuration', async () => {
        ...
        const checkConfig = function(pbcfg: GithubAppPrebuildConfig, expectation: boolean[]) {
            ...
            chai.assert.equal(rules.shouldRunPrebuild(cfg, true, false, false), expectation[0], `master prebuild check failed with ${JSON.stringify(pbcfg)}`);
            chai.assert.equal(rules.shouldRunPrebuild(cfg, false, true, false), expectation[1], `PR prebuild check failed with ${JSON.stringify(pbcfg)}`);
            chai.assert.equal(rules.shouldRunPrebuild(cfg, false, false, false), expectation[2], `branch prebuild check failed with ${JSON.stringify(pbcfg)}`);
            chai.assert.equal(rules.shouldRunPrebuild(cfg, false, true, true), expectation[3], `PR from Fork prebuild check failed with ${JSON.stringify(pbcfg)}`);
        }
