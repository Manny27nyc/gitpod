...
export class TeamSubscriptionService {
     ...
     * @param date
     */
    async assignSlot(teamSubscription: TS, slotId: string, assignee: User, assigneeIdentifier: AssigneeIdentifier, date: Date) {
        return this.accountingDb.transaction(async (db) => {
            ...
            switch (state) {
                case 'unassigned':
                    subscription = await this.addSubscription(db, assignee.id, ts.planId, slot.id, startDate, Plans.getHoursPerMonth(plan));
                    break;
                case 'deactivated':
                    ...
                    const remainingHoursThisPeriod = await this.calculateRemainingHoursThisPeriod(db, slot, startDate, now, Plans.getHoursPerMonth(plan));
                    subscription = await this.addSubscription(db, assignee.id, ts.planId, slot.id, startDate, Plans.getHoursPerMonth(plan), remainingHoursThisPeriod, endDate, cancelationDate);
                    break;
                default:
            ...
            log.info({ userId: ts.userId }, "Assigned team subscription slot", { teamSubscription: ts, slotId, assignee, date })
        });
    }
    ...
    async reactivateSlot(teamSubscription: TS, slotId: string, date: Date) {
        return this.accountingDb.transaction(async (db) => {
            ...
            if (slot.assigneeId) {
                const plan = Plans.getById(ts.planId)!;
                const subscription = await this.addSubscription(db, slot.assigneeId, ts.planId, slot.id, startDate, Plans.getHoursPerMonth(plan));
     ...
     * @param date
     */
    async reassignSlot(teamSubscription: TS, slotId: string, newAssignee: User, newAssigneeIdentifier: AssigneeIdentifier, date: Date) {
        return this.accountingDb.transaction(async (db) => {
            const now = date.toISOString();
            const ts = await this.getTeamSubscription(teamSubscription);
            const { startDate } = Subscription.calculateCurrentPeriod(ts.startDate, date);
            ...
            const plan = Plans.getById(ts.planId)!;
            const remainingHoursThisPeriod = await this.calculateRemainingHoursThisPeriod(db, slot, startDate, now, Plans.getHoursPerMonth(plan));
            const newSubscription = await this.addSubscription(db, newAssignee.id, ts.planId, slot.id, startDate, Plans.getHoursPerMonth(plan), remainingHoursThisPeriod);

            // Update slot
            TeamSubscriptionSlot.assign(slot, newAssignee.id, newSubscription.uid, newAssigneeIdentifier);
            await this.db.storeSlot(slot);
            log.info({ userId: ts.userId }, "Reassigned team subscription slot", { teamSubscription: ts, slotId, newAssignee, date })
        });
    }
    ...
    protected async calculateRemainingHoursThisPeriod(db: AccountingDB, slot: TeamSubscriptionSlot, currentPeriodStartDate: string, now: string, hoursPerMonth: number) {
        ...
        const calcTotalUsage = async (subscriptions: Subscription[], userId: string) => {
            ...
            return statement.credits
                ...
                .filter(c => CreditDescription.is(c.description) && subscriptions.some(s => s.uid === (c.description as CreditDescription).subscriptionId))
        ...
        userMap.forEach((subscriptions, userId) => { totalUsagesPerUser.push(calcTotalUsage(subscriptions, userId)); });
    ...
    async findUnassignedSlots(teamSubscriptionId: string): Promise<TeamSubscriptionSlot[]> {
        ...
        if (ts.deleted ||
            !!ts.cancellationDate && ts.cancellationDate < now) {
            return [];
        }
        return (await this.db.findSlotsByTeamSubscriptionId(teamSubscriptionId)).filter(s => TeamSubscriptionSlot.status(s, now) === 'unassigned');
    }
    ...
    protected async findAssignedSubscriptionsBy(teamSubscription: TeamSubscription, date: Date) {
        ...
        return Promise.all(slots.map(async (s) => {
            const subscription = s.subscriptionId ? await this.accountingDb.findSubscriptionById(s.subscriptionId) : undefined;
            const remainingHoursThisPeriod = await this.calculateRemainingHoursThisPeriod(this.accountingDb, s, startDate, dateStr, Plans.getHoursPerMonth(plan));
    ...
    protected async loadSlot(id: string, tsId: string, now: string, ...expectedStates: TeamSubscriptionSlotState[]): Promise<{ slot: TeamSubscriptionSlot, state: TeamSubscriptionSlotState }> {
    ...
    protected async cancelSubscription(db: AccountingDB, assigneeId: string, planId: string, slotId: string, cancellationDate: string) {
    ...
    protected async addSubscription(db: AccountingDB, assigneeId: string, planId: string, slotId: string, startDate: string, amount: number, firstMonthAmount?: number, endDate?: string, cancelationDate?: string) {
        const model = await this.loadSubscriptionModel(db, assigneeId);
